"""Orchestrator Agent - Generates Business PRDs through conversation.

This agent engages with users to understand their application requirements
and generates comprehensive Product Requirements Documents (PRDs).
"""
...
class OrchestratorAgent(ContextAwareAgent):
    """Context-aware orchestrator agent for PRD generation."""
    ...
    async def generate_prd(self, user_prompt: str, skip_questions: bool = False, **kwargs) -> dict[str, Any]:
        """Generate a Business PRD through conversation.
        
        Args:
            user_prompt: Initial user request
            skip_questions: If True, generate PRD without asking questions
            **kwargs: Additional options
            
        Returns:
            Dictionary containing:
                - success: Whether PRD was generated successfully
                - prd_content: The generated PRD content
                - app_name: Extracted application name
                - conversation_turns: Number of conversation turns
                - cost: Total cost of generation
        """
        logger.info(f"Starting PRD generation for: {user_prompt[:100]}...")
        if skip_questions:
            logger.info("Skip questions mode: Will generate PRD directly without conversation")
        ...
        if result.success:
            ...
            if not prd_content and result.tool_uses:
                for tool_use in result.tool_uses:
                    if tool_use.get("name") == "Write" and tool_use.get("input", {}).get("content"):
                        # Extract content from Write tool
                        prd_content = tool_use["input"]["content"]
                        logger.info("Extracted PRD content from Write tool usage")
                        break
            
            # Extract app name from the PRD (simple heuristic - can be improved)
            app_name = self._extract_app_name(prd_content, user_prompt)
    ...
    def _extract_app_name(self, prd_content: str, user_prompt: str) -> str:
        """Extract application name from PRD content.
        ...
        """
        # First try to extract from PRD title
