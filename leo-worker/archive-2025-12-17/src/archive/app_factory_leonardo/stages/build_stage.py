...
TEMPLATE_PATH = Path.home() / ".mcp-tools" / "templates" / TEMPLATE_NAME


def cleanup_template_files(app_dir: Path) -> None:
    """Remove template placeholder files that agents will regenerate.
    
    This ensures agents create files from scratch rather than trying to
    modify existing templates, which causes confusion and permission issues.
    ...
    files_to_remove = [
        ...
        # Don't remove App.tsx as it has routing setup that's useful
    ]
...
async def run_stage(
    ...
    **kwargs
    ...
    try:
        ...
        main_page_path = app_dir / "client" / "src" / "pages" / "HomePage.tsx"
