...
/**
 * Docker Manager for Leo Container Orchestration
 *
 * Manages the lifecycle of Leo container instances for app generation.
 * Each container runs the leo-websocket image and connects to the WebSocket server
 * to receive generation commands and report progress.
 *
 * Architecture:
 * - One container per generation request
 * - Containers connect to WebSocket server at ws://host:port/ws/job_{requestId}
...
export interface ContainerSpawnConfig {
  ...
  appName: string;
  /** User ID (UUID) - for GitHub repo naming */
  userId?: string;
...
export class DockerManager extends EventEmitter {
  ...
  constructor() {
    ...
    // WebSocket URL: Passed from WSI Server or use default
    // For leo-remote: WSI Server runs locally, container connects via host.docker.internal
    // For leo-saas: WSI Server runs in orchestrator, container connects via ALB
    this.WS_BASE_URL = process.env.WS_BASE_URL || 'ws://host.docker.internal:5013';
   ...
   * The container will connect to the WebSocket server and await generation commands.
   *
  ...
  async spawnContainer(config: ContainerSpawnConfig): Promise<string> {
    const { requestId, prompt, mode, maxIterations, appName, userId, appId, githubOwner, isResume, resumeFromPath, resumeSessionId, outputDir } = config;
    ...
    try {
      ...
      // Construct WebSocket URL for this specific job
      const wsUrl = `${this.WS_BASE_URL}/ws/job_${requestId}`;

      // Acquire Supabase credentials from pool manager
      const supabasePool = getSupabasePoolManager();
      let supabaseCreds: SupabaseCredentials;
      ...
      const env = [
        ...
        // User and App IDs for GitHub repo naming
        `USER_ID=${userId || os.userInfo().username}`,
      ...
      const containerConfig: Docker.ContainerCreateOptions = {
        ...
        HostConfig: {
          ...
          Binds: [
            ...
            // Note: Container runs as leouser (UID 1000), so mount to /home/leouser/.aws
            `${process.env.HOME}/.aws:/home/leouser/.aws:ro`,
          ],
