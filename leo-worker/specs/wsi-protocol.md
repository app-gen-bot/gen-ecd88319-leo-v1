...
| **Client** | `user_input`          | **NEW in v2.2** - Mid-generation user input | User types while agent works               |
...
#### `user_input` **NEW in v2.2**

**Origin:** Client (Orchestrator/UI)
**Timing:** Sent during active generation to inject additional guidance
**Description:** Allows users to send additional prompts or guidance to the agent while it's actively working, similar to typing in the Claude Code CLI while the agent is running.

**Fields:**

| Field     | Type   | Required | Description                           |
| --------- | ------ | -------- | ------------------------------------- |
| `type`    | string | ✓        | Always `"user_input"`                 |
| `content` | string | ✓        | User's message to inject into agent   |

**Example:**
```json
{
  "type": "user_input",
  "content": "Also add form validation to the login page"
}
```

**Example (redirect focus):**
```json
{
  "type": "user_input",
  "content": "Focus on fixing the TypeScript errors first before adding new features"
}
```

**Behavior:**
- Messages are queued and processed sequentially by the agent
- The agent receives the message as additional context in its conversation
- Works during any generation mode (interactive, confirm_first, autonomous)
- No response message is sent back; the agent's subsequent output reflects the input
- If no generation is active, the message is ignored (container logs a warning)

**CLI Equivalent:**
In the Claude Code CLI, users can type while the agent is working. The input is queued and processed when the agent is ready for new instructions.
