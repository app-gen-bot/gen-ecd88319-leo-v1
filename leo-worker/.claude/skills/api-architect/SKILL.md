...
### 7. Flat Method Names (No Nesting)

**Problem**: Guessing nested patterns like `apiClient.store.items.list()`.

```typescript
// ✅ CORRECT: Flat methods
apiClient.orders.list()       // list
apiClient.orders.getById()    // getById
apiClient.orders.create()     // create

// ❌ WRONG: Nested (doesn't exist)
apiClient.orders.items.list()
```
...
**Rule**: Always check contract file for exact method name before writing `apiClient` calls.

---

### 8. Route Ordering (Static Before Dynamic)

**Problem**: `/mine` after `/:id` causes 400 errors.

```typescript
// ❌ WRONG
getById: { path: '/items/:id', ... },
getMine: { path: '/items/mine', ... },  // Never reached!

// ✅ CORRECT
getMine: { path: '/items/mine', ... },  // Static first
getById: { path: '/items/:id', ... },   // Dynamic after
```

**Rule**: Static paths before dynamic paths at the same level.
...
2. **Create common.ts** → Shared error schema
3. **Create auth contract** → login, signup, logout
4. **Create entity contracts** → CRUD for each entity
5. **Create index.ts** → Combine all contracts
6. **Create routes** → Implement handlers with storage
7. **Verify** paths have no /api prefix
...
### Common Schema (shared/contracts/common.ts)
...
// Use relative URL - works in both dev (Vite proxy) and prod (same origin)
// Dev: Vite proxy forwards /api/* to backend server
// Prod: Express serves both frontend static files and API from same origin
export const apiClient = initClient(contract, {
  baseUrl: '/api',
  jsonQuery: true,
  baseHeaders: {
...
**Important**: The Vite config MUST include a proxy for development:
...
- [ ] Contract methods use flat naming (list, getById, create)
- [ ] Static paths before dynamic (`/mine` before `/:id`)
...
| Wrong route order | Static paths unreachable | Static before dynamic |
