...
# Container path for code_writer patterns (co-located with agent)
PATTERNS_DIR = Path("/factory/leo/agents/app_generator/subagents/code_writer/patterns")
...
PATTERN_FILES = {
    ...
    "server_mounting": PATTERNS_DIR / "SERVER_MOUNTING.md",
...
14. **Form State Management**: {PATTERN_FILES['form_state']} (prevents production data loss)
15. **Auth Signup Pattern**: {PATTERN_FILES['auth_signup']} (prevents duplicate user creation)
16. **Server Mounting & ts-rest**: {PATTERN_FILES['server_mounting']} (prevents server crashes)
